        ...
        if rename:
            ...
            if candidate.resolve() == src.resolve():
                plan.append({
                    ...
                    "skipped": True, "reason": "already organized",
                    "duplicate_of": None, "file_hash": file_hash,
                })
                continue
            n, stem, ext = 1, src.stem, src.suffix
            while candidate in used_dsts or candidate.exists():
...
class MoApp(ctk.CTk):
    ...
    def _populate_tree(self, plan, sort_mode):
        ...
        already_organized = [p for p in skipped if p["reason"] == "already organized"]
        other_skipped = [
            p for p in skipped
            if p["reason"] not in ("duplicate", "no date", "already organized")
        ]
        ...
        if already_organized:
            ao_id = "already_organized"
            self.tree.insert(
                "",
                "end",
                iid=ao_id,
                text=f"✓ Already Organized ({len(already_organized)})",
                values=("", "", "", ""),
                tags=("skipped",),
                open=False,
            )
            for item in already_organized:
                self.tree.insert(
                    ao_id,
                    "end",
                    text=item["src"].name,
    ...
    def _populate_date_tree(self, items):
        ...
        for year in sorted(by_year):
            year_id = f"year::{year}"
            year_count = sum(len(v) for v in by_year[year].values())
            self.tree.insert("", "end", iid=year_id,
                             text=f"{year}  ·  {year_count} files",
                             values=("", "", "", ""), open=True)
            for month in sorted(by_year[year], key=MONTH_NAMES.index):
                month_items = by_year[year][month]
